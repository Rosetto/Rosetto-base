...
/**
 * タグ文字列をRosettoActionに変換するためのパーサー.
 * @author tohhy
 */
public abstract class AbstractTagParser {
    
    /**
     * 指定タグを変換する.parseTagから呼び出される.
     * 個々のエンジンごとの独自関数とその引数を受け取り、
     * Rosettoの関数を呼び出すRosettoActionに変換して返すように実装する.
    ...
    /**
     * 指定文字列を囲んでいる角括弧を除去する.
     * 行頭と行末にそれぞれ左角括弧と右角括弧が存在する場合にその再外周の括弧のみを取り除く.
    ...
    /**
     * タグを受け取り、タグの名前部分を返す.
     * 角括弧で囲まれていればそれを外し、最初に登場するスペースの直前までをタグ名とみなして返す.
     * 行頭と行末のホワイトスペースは無視される.
     * @param tag タグ
     * @return タグの名前部分
     */
